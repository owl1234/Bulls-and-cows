def handler(event, context):
    ...
    Навык был создан 2 года назад. На момент отправки карточки грантовика он работал так же, как и работал 2 года назад. Буквально на днях мне написал Яндекс, 
    сказав о том, что в работе навыка есть перебои + правила публикации немного изменились, в связи с чем необходимо немного изменить навык. Соответственно, 
    я подправила его и развернула его не на платформе Now (как раньше), а на платформе Яндекс.Облако, где не требуется импортировать вспомогательные модули, 
    а нужен только код, в котором происходит взаимодействие с Алисой, поэтому:
    1. Навык висит на проверке модерации (первая ссылка на навык в каталоге навыков не работает, поскольку его на днях отозвали с просьбой подправить; 
        доказательство: https://sun9-12.userapi.com/BU8LqnyFDWzN8CYbgX-Z01liyThiQ8l7e8m7nw/egusJUAhdM8.jpg);
    2. В навыке всё прекрасно работает, ничего не утеряно;
    3. Его только что одобрили (20.08.2002 14:55). Ссылка в презентации нерабочая, вот новая: https://dialogs.yandex.ru/store/skills/89857802-byki-i-korovy
    Спасибо за внимание!
    !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    """
